...
if uploaded_file is not None:
    ...
    video_data["month"] = (
        pd.to_datetime(video_data["Post_Date"]).dt.to_period("M").dt.start_time
    )
    ...
    date_start = st.sidebar.date_input(
        "開始日",
        value=video_data["Post_Date"].min(),
        min_value=video_data["Post_Date"].min(),
    )
    date_end = st.sidebar.date_input("終了日")
    view_min = st.sidebar.number_input("再生数下限", step=10000)
    like_min = st.sidebar.number_input("いいね数下限", step=1000)

    # 設定に合わせてデータをフィルター
    data = video_data.query(
        "@date_end >= Post_Date >= @date_start & Views > @view_min & Likes > @like_min"
    )
    ...
    st.subheader("月別の再生数")
    chart_data = data[
        ["Post_Date", "month", "Video_Title", "Views", "Likes", "Comments"]
    ]
    st.bar_chart(chart_data, x="Video_Title", y=["Views", "Likes"])  # ★演習3
    ...
    st.dataframe(
        count_df.sort_values(by="count", ascending=False).head(10), hide_index=True
    )

    # 動画別データの一覧表示
    st.subheader("動画別のデータ")
    st.dataframe(
        data[["thumbnails", "Video_Title", "Views", "Likes", "Comments"]],
        column_config={
